...
/*      */     }
/*  509 */     this.methodParametersCount += 1;
...
/*  628 */           Type.getArgumentTypes(this.descriptor), 
/*  628 */           nLocal);
...
/*      */       case 3:
/*  713 */         if (delta < 64)
/*  714 */           this.stackMap.putByte(delta);
/*      */         else {
...
/*      */       {
/*  800 */         this.currentBlock.status |= 256;
/*      */ 
...
/* 1025 */           label.status |= 512;
/* 1026 */           this.subroutines += 1;
/*      */         }
/* 1028 */         this.currentBlock.status |= 128;
...
/* 1272 */         dflt.getFirst().status |= 16;
/* 1273 */         for (int i = 0; i < labels.length; ++i) {
/* 1274 */           addSuccessor(0, labels[i]);
/* 1275 */           labels[i].getFirst().status |= 16;
/*      */         }
/*      */       }
/*      */       else {
/* 1279 */         this.stackSize -= 1;
/*      */ 
...
/*      */       }
/* 1382 */       this.localVarTypeCount += 1;
/* 1383 */       this.localVarType.putShort(start.position)
/* 1384 */         .putShort(end.position - start.position)
/* 1385 */         .putShort(this.cw
/* 1385 */         .newUTF8(name))
/* 1385 */         .putShort(this.cw.newUTF8(signature))
/* 1386 */         .putShort(index);
/*      */     }
/*      */ 
...
/* 1457 */         Label l = handler.start.getFirst();
...
/*      */       {
/* 1497 */         Label l = changed;
/* 1498 */         changed = changed.next;
/* 1499 */         l.next = null;
/* 1500 */         f = l.frame;
/*      */ 
/* 1502 */         if ((l.status & 0x10) != 0) {
/* 1503 */           l.status |= 32;
/*      */         }
/*      */ 
/* 1506 */         l.status |= 64;
/*      */ 
/* 1508 */         int blockMax = f.inputStack.length + l.outputStackMax;
/* 1509 */         if (blockMax > max) {
/* 1510 */           max = blockMax;
/*      */         }
/*      */ 
/* 1513 */         Edge e = l.successors;
/* 1514 */         while (e != null) {
...
/* 1549 */             this.frame[frameIndex]
/* 1550 */                = (0x1700000 | this.cw
...
/*      */             {
/* 1616 */               ++id;
/* 1617 */               subroutine.visitSubroutine(null, id / 32L << 32 | 1L << id % 32, this.subroutines);
/*      */             }
/*      */           }
/*      */ 
/* 1621 */           l = l.successor;
...
/*      */           {
/* 1674 */             l.inputStackTop = ((b.info == 2147483647) ? 1 : start + b.info);
/*      */ 
/* 1677 */             l.status |= 8;
/* 1678 */             l.next = stack;
/* 1679 */             stack = l;
...
/*      */ 
/* 1756 */     for (int i = 0; i < locals.length; ++i) {
/* 1757 */       int t = locals[i];
/* 1758 */       if (t == 16777216) {
/* 1759 */         ++nTop;
/*      */       } else {
...
/*      */           }
/*      */         }
/* 1839 */         this.frame[(frameIndex++)]
/* 1840 */            = (0x1700000 | this.cw
/* 1840 */           .addType(this.descriptor
/* 1840 */           .substring(j, ++i)));
/*      */ 
/* 1841 */         break;
/*      */       case 'L':
...
/* 1844 */           ++i;
/*      */         }
/* 1846 */         this.frame[(frameIndex++)]
/* 1847 */            = (0x1700000 | this.cw
/* 1847 */           .addType(this.descriptor
/* 1847 */           .substring(j + 1, i++)));
/*      */ 
/* 1848 */         break;
/*      */       case 'E':
/*      */       case 'G':
...
/*      */   {
/* 1900 */     int clocalsSize = this.frame[1];
...
/* 2273 */             .putShort(h.handler.position)
/* 2273 */             .putShort(h.type);
/* 2274 */           h = h.next;
/*      */         }
/*      */       }
...
/* 2352 */         .putShort(this.cw
/* 2352 */         .newUTF8(this.signature));
/*      */     }
/*      */ 
 ...
 * Qualified Name:     org.jacoco.asm.MethodWriter
