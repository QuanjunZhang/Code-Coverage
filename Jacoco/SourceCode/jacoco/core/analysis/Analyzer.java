...
public class Analyzer {
	...
	//最终调用这个
	//核心
	//在reporttask.java中，将classfiles传递过来，调用该函数
	private ClassVisitor createAnalyzingVisitor(final long classid,
			final String className) {
		//根据classid重executionData中获得executionData
		//这里我们可以看到，首先定义了ExecutionData对象
		//之后使用了ExecutionDataStore.get函数实例化上面的对象
		final ExecutionData data = executionData.get(classid);

		final boolean[] probes;

		final boolean noMatch;//判断classid是否与executionData匹配，不匹配为true
		if (data == null) {//如果根据该classID得到的executionData为空，那么probes也是空的，
			probes = null;
			noMatch = executionData.contains(className);//判断该类是否有executionData在ExecutionDataStore里面
		} else {
			probes = data.getProbes();//传入探针数组
			noMatch = false;
		}

		//ClassCoverageImpl:Coverage data of a single class containing methods.
		final ClassCoverageImpl coverage = new ClassCoverageImpl(className,
				classid, noMatch);
        //核心 继承自classProbeVisitor
		...
		final ClassAnalyzer analyzer = new ClassAnalyzer(coverage, probes,
			...
			public void visitEnd() {
				super.visitEnd();
				//来自CoverageBuilder implements ICoverageVisitor
	 ...
	 */
	//下方的函数将inputstream转换为buffer，再调用的是这个
